...
async function seedProducts(client: any, categoryIds: string[]): Promise<string[]> {
   ...
   for (const product of products) {
      const result = await client.query(`
         INSERT INTO product (name, description, price, stock, category_id, sku)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING product_id
      `, [product.name, product.description, product.price, product.stock, product.categoryId, product.sku]);
...
async function seedOrderItems(client: any, orderIds: string[], productIds: string[]) {
   ...
   const orderItems = [
      ...
      { orderId: orderIds[0], productId: productIds[4], quantity: 1, price_at_time: 150.00 },
